...
abstract class Common extends \Common {
    ...
    public function __construct() {
        ...
        if ($config?->tg?->log_file) {
            $this->log_dir = mb_substr($config?->tg?->log_file, 0, 1) != '/'
                ? realpath(DOC_ROOT . dirname($this->config?->log?->path))
                : dirname($this->config?->log?->path);
        ...
        $this->settings->setSerialization(
            (new MadelineProto\Settings\Serialization())
                ->setInterval(30)
        );
//        $this->settings->setDb(
//            (new MadelineProto\Settings\Database\Memory())->setEnableMinDb(false)
//        );
        ...
        $this->session_file = (string)$session_file;
    }
